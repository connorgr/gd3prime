  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (showDuplicates){
        ...
        var allSampleNames = {};
        if (Object.keys(typeToSamples).length > 0) {
          console.log(typeToSamples)
      ...
      function distanceMeasure() {
        // Get the ordering of samples in the matrix
        sampleToIndex

        sampleMutations.sort(function(s1,s2) {
          var a = sampleToIndex[s1.name],
              b = sampleToIndex[s2.name];
        ...
        var listOfSummaries = [],
            summary = [];
        for (var s in sampleMutations) {
          var sample = sampleMutations[s];
          if(s == 0) {
            summary.push(sample);
          } else {
            var prevSample = sampleMutations[s-1],
                s1Len = sample.genes.length,
                s2Len = prevSample.genes.length;
            if (sample.genes.length != prevSample.genes.length) {
              listOfSummaries.push(summary);
              summary = [];
              console.log(s1Len,s2Len,sample.name,prevSample.name);
            } else {
              summary.push(sample);
            }
          }
        ...
        for (var i in listOfSummaries) {
          var samples = listOfSummaries[i];
          if (samples.length < 4) {
            continue;
          }
